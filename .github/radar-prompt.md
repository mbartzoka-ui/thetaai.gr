...
- https://www.theguardian.com/technology/artificialintelligenceai/rss   (γενική κάλυψη AI: πιάνει έρευνα και κοινωνικά θέματα που τα feeds ασφαλείας αγνοούν)
- https://openai.com/blog/rss.xml
- https://blog.google/technology/ai/rss/
- https://www.anthropic.com/news

STEP 2 — FILTER. Keep ONLY items relating to: EU AI Act / AI Act / Article 50 / high-risk AI / GPAI / AI regulation-governance-compliance; AI security / LLM security / prompt injection / jailbreak / adversarial AI / AI-agent security; ISO 42001 / ISO/IEC 42001 / ISO 27001+AI / AI management systems / AI standards; NIST AI / AI RMF / AI risk; GDPR+AI / DPIA+AI / AI data protection; AI incidents / chatbot liability / AI lawsuits / major data-breach fines; AND notable policy/safety/security announcements from major AI labs (Anthropic, OpenAI, xAI/Grok, Google DeepMind, Meta, Microsoft);

AND — προσθήκη 17/09/2026 — ΑΝΑΚΟΙΝΩΣΕΙΣ ΠΡΟΪΟΝΤΟΣ Η ΛΕΙΤΟΥΡΓΙΑΣ ΠΟΥ ΖΗΤΟΥΝ ΝΕΑ ΠΡΟΣΒΑΣΗ ΣΕ ΕΥΑΙΣΘΗΤΑ ΔΕΔΟΜΕΝΑ. Αν ένα AI εργαλείο, από οποιονδήποτε πάροχο, ζητά σύνδεση με τραπεζικούς λογαριασμούς, δεδομένα υγείας, email, ημερολόγιο, αρχεία, επαφές, τοποθεσία, CRM ή συστήματα πληρωμών, ΜΠΑΙΝΕΙ. Το ίδιο αν μετακινεί όριο εμπιστοσύνης: νέα δικαιώματα σε agent, πρόσβαση σε εταιρικά συστήματα, ή λειτουργία που δεν κυκλοφορεί στην ΕΕ λόγω νομοθεσίας προστασίας δεδομένων.
ΓΙΑΤΙ ΥΠΑΡΧΕΙ: στις 17/09/2026 το «Claude Money» της Anthropic, όπου ο χρήστης συνδέει τον τραπεζικό του λογαριασμό στο Claude, ΗΤΑΝ στο feed του BleepingComputer τέσσερις ώρες πριν τρέξει το πέρασμα, και κόπηκε επειδή είναι ανακοίνωση ΠΡΟΪΟΝΤΟΣ και όχι policy/safety. Το γράμμα του κανόνα τηρήθηκε, η ουσία χάθηκε: μια εταιρεία AI που ζητά πρόσβαση στον τραπεζικό λογαριασμό είναι ακριβώς το θέμα της Maria, και το ίδιο το ρεπορτάζ σημείωνε ότι ίσως δεν κυκλοφορήσει στην Ευρώπη λόγω νόμων απορρίας. Η Maria το εντόπισε μόνη της και ρώτησε γιατί λείπει. Prefer items from the last ~3 weeks — αυτό ισχύει ΜΟΝΟ για το ΑΡΧΕΙΟ του site (STEP 3-6), όπου το βάθος είναι το ζητούμενο.
...
ΕΛΕΓΧΟΣ ΘΕΜΑΤΟΣ, ΟΧΙ ΜΟΝΟ ΠΕΡΙΣΤΑΤΙΚΟΥ — ρητή παρατήρηση της Maria 17/09/2026: «η ιστορία επαναλαμβάνεται απλά από άλλο incident».
Ο έλεγχος επανάληψης ως τώρα έπιανε το ΙΔΙΟ ΓΕΓΟΝΟΣ. Δεν έπιανε το ΙΔΙΟ ΘΕΜΑ με άλλο γεγονός, και αυτό είναι που κουράζει τον αναγνώστη.
ΘΕΜΑ = η υποκείμενη ερώτηση, όχι το συμβάν. Παραδείγματα από τα digests 15-17/09/2026, όπου το ίδιο θέμα πέρασε τρεις μέρες στη σειρά με τρία διαφορετικά γεγονότα:
- «agents που αναφέρουν ή καταδίδουν άλλους agents»: DeepMind 15/09, γραμμές καταγγελίας 16/09, πλαίσιο αναφοράς OpenAI 17/09.
- «AI βοηθός μέσα σε browser υπό επίθεση»: δύο ξεχωριστές ιστορίες στο ΙΔΙΟ digest της 17/09.
ΚΑΝΟΝΕΣ:
1. ΜΕΣΑ στο ίδιο digest, ΜΟΝΟ ΜΙΑ ιστορία ανά θέμα. Αν δύο γεγονότα απαντούν στην ίδια ερώτηση, κράτα το ισχυρότερο και ανάφερε το άλλο σε μία φράση μέσα στην ίδια ενότητα.
2. Πριν μπει θέμα, κοίτα τα ΤΕΛΕΥΤΑΙΑ 7 digests (GitHub issues με τίτλο «AI Radar Digest»). Αν το ίδιο θέμα έχει εμφανιστεί ΔΥΟ ή περισσότερες φορές εκεί, ΔΕΝ ξαναμπαίνει, εκτός αν το νέο γεγονός αλλάζει την απάντηση στην ερώτηση και όχι μόνο το παράδειγμα.
3. Αν το βάλεις παρόλα αυτά, ΓΡΑΨΕ ΡΗΤΑ στην ενότητα γιατί δεν είναι επανάληψη, σε μία πρόταση. Αν δεν μπορείς να τη γράψεις, είναι επανάληψη.
ΠΡΟΤΙΜΗΣΗ ΠΟΙΚΙΛΙΑΣ: όταν οι υποψήφιες φρέσκιες ιστορίες καλύπτουν λιγότερα θέματα από τις θέσεις του digest, γράψε ΛΙΓΟΤΕΡΕΣ ιστορίες. Τρεις ιστορίες σε τρία θέματα αξίζουν περισσότερο από έξι σε δύο.
